...
class MapGenerator3D:
    ...
    def generate(self):
        for _ in range(_MAX_RETRIES):
            grid = np.zeros((self.depth, self.height, self.width), dtype=np.uint8)
        ...
        raise RuntimeError(f"Could not generate a solvable 3D map after {_MAX_RETRIES} attempts")

    def generate_many(self, num_maps: int, output_dir: str = None) -> list:
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        folder = output_dir or f"3dblock_map_{timestamp}"
    ...
    def _place_obstacle(self, grid: np.ndarray):
        size = min(random.randint(*self.obstacle_size_range), self.width, self.height, self.depth)
        x = random.randint(0, self.width - size)
        y = random.randint(0, self.height - size)
        z = random.randint(0, self.depth - size)
        grid[z:z + size, y:y + size, x:x + size] = 1
